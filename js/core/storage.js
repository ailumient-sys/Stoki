...
/* =========================================================
   MIGRACIONES
   ========================================================= */
function migrateToTipos(){
  /* Cada producto existente pasa a tipo 'producto' con unidad 'unidad'.
     Los tipos de margen viejos se mapean a los nuevos. */

  window.DB.products = (window.DB.products || []).map(p => {
    /* Tipo */
    if(!p.tipo) p.tipo = 'producto';

    /* Unidad */
    if(!p.unidad) p.unidad = 'unidad';

    /* Stock infinito */
    if(typeof p.stockInfinito === 'undefined') p.stockInfinito = false;

    /* Vendible suelto (solo aplica a materiales) */
    if(typeof p.vendibleSuelto === 'undefined') p.vendibleSuelto = false;

    /* También material (producto que se usa en recetas) */
    if(typeof p.tambienMaterial === 'undefined') p.tambienMaterial = false;

    /* Migrar tipo de margen viejo → nuevo */
    const tipoViejo = p.tipoMargen;
    if(tipoViejo && tipoViejo !== 'pct-unidad' && tipoViejo !== 'fijo-unidad' && tipoViejo !== 'fijo-lote' && tipoViejo !== 'precio-fijo'){
      if(tipoViejo === 'porcentaje'){
        p.tipoMargen = 'pct-unidad';
      } else if(tipoViejo === 'fijo'){
        p.tipoMargen = 'fijo-unidad';
      } else if(tipoViejo === 'fijo-lote'){
        p.tipoMargen = 'fijo-lote';
      } else if(tipoViejo === 'precio' || tipoViejo === 'precio-lote'){
        p.tipoMargen = 'precio-fijo';
      } else {
        p.tipoMargen = 'pct-unidad';
      }
    }
    ...
    /* Receta: componentes */
    if(p.tipo === 'receta' && !Array.isArray(p.componentes)){
      p.componentes = [];
    }

    /* Servicio: consumibles */
    if(p.tipo === 'servicio' && !Array.isArray(p.consumibles)){
      p.consumibles = [];
    }
  ...
  /* Inicializar arrays faltantes del DB */
  if(!Array.isArray(window.DB.movimientos)) window.DB.movimientos = [];

  console.log('✅ Migración v15 completa: tipos agregados');
}
...
async function correrMigraciones(){
  ...
  if(window.DB.version < 15) migrateToTipos();
